...
<html>
	...
	<body>
		<?php
			...
			/* Find meetings that have not yet passed and the user is a part of but not a host (i.e. isHost = 0) */
			$meeting_query = "select * from project.meeting where timepassed = 0 and meeting_id in (select meeting_id from project.meeting_members where user_id = '$user_id' and isHost = 0) order by meeting_datetime";
			$meeting_result = mysqli_query($connect,$meeting_query);
			$num_rows = mysqli_num_rows($meeting_result);
	
			echo "
				<p align = 'center'>Upcoming Meetings Where I am the Guest of</p>
				<br>
				<br>
			";

			/*If there are no meetings, show 'You currently do not have any upcoming meetings in which you are the guest' */
			if ($num_rows == 0) {
				echo "
					<p align='center'>You currently are not a guest of any meeting <br> Go to 'Explore!' and explore new meetings now!</p>
				";
			}
					...
					<table width='700' align='center' cellspacing='0' border='1'>
						...
						<tr>
							<td colspan='2' align='center' height='30'>
								<button type='submit' formaction='meeting_leave.php'>Leave this Meeting</button> &nbsp;
							</td>
						</tr>
					</table>
				</form>
